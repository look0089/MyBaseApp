...
public class ChangeCharActivity extends Activity implements OnClickListener {
    ...
    BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context arg0, Intent intent) {
            ...
            if (BleService.ACTION_GATT_DISCONNECTED.equals(action)) {
                Toast.makeText(ChangeCharActivity.this, "设备连接断开", Toast.LENGTH_SHORT).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences = this.getSharedPreferences("writedata", MODE_PRIVATE);
    ...
    private void init() {
        ...
        charUuid = UUID.fromString(getIntent().getExtras().get("charUUID").toString());
        serUuid = UUID.fromString(getIntent().getExtras().get("serUUID").toString());
    ...
    private void saveResualt(String result) {
        if (result.isEmpty()) {
            ...
            try {
                ...
                writer.close();
                out.close();
                Toast.makeText(ChangeCharActivity.this, "BLElog" + System.currentTimeMillis()
                        + ".txt文件成功保存到SD卡根目录下", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void writeDialog() {
        ...
        View dialogview = LayoutInflater.from(this).inflate(R.layout.writedialog, null);
        final RadioGroup writeGroup = (RadioGroup) dialogview.findViewById(R.id.rg_write_format);
        final EditText editText = (EditText) dialogview.findViewById(R.id.char_value);
        final Button timing_write = (Button) dialogview.findViewById(R.id.btn_timing_write);
        ...
        final TextView timing_write_count = (TextView) dialogview.findViewById(R.id.tv_timing_write_count);
        final RadioButton radio1 = (RadioButton) dialogview.findViewById(R.id.rb_write_string);
        final RadioButton radio2 = (RadioButton) dialogview.findViewById(R.id.rb_write_hex);
        final EditText editbtn1 = (EditText) dialogview.findViewById(R.id.ed_edit_btn1);
        final EditText editbtn2 = (EditText) dialogview.findViewById(R.id.ed_edit_btn2);
        final EditText editbtn3 = (EditText) dialogview.findViewById(R.id.ed_edit_btn3);

        AppLog.e("TAG", "writeDialog: "+charUuid);
                        ...
                        try {
                            while (writing) {
                                ...
                                if (!charvalue.isEmpty()) {
                                    if (isHex) {
                                        write_byte_number += charvalue.length() / 2 + charvalue.length() % 2;
                                    ...
                                    runOnUiThread(new Runnable() {
                                        ...
                                        @Override
                                        public void run() {
                                            timing_write_count.setText("共:" + write_byte_number);
                                        }
                                    });
                                }
                            }
