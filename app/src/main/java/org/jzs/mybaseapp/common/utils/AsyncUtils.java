...
public class AsyncUtils {
    public static ExecutorService backgroundExecutor = Executors.newScheduledThreadPool(2 * Runtime.getRuntime().availableProcessors());
    private static Function<Throwable> crashLogger = throwable -> throwable.printStackTrace();
    ...
    public static <T> Future<?> doAsync(T t, Function<AsyncContext<T>> task) {
        return doAsync(t, crashLogger, backgroundExecutor, task);
    }

    public static <T> Future<?> doAsync(T t, ExecutorService executorService, Function<AsyncContext<T>> task) {
        return doAsync(t, crashLogger, executorService, task);
    }

    public static <T> Future<?> doAsync(T t, Function<Throwable> exceptionHandler, Function<AsyncContext<T>> task) {
        return doAsync(t, exceptionHandler, backgroundExecutor, task);
    }

    public static <T> Future<?> doAsync(T t, final Function<Throwable> exceptionHandler, ExecutorService executorService, final Function<AsyncContext<T>> task) {
     ...
     */
    public static <R> R forceAsync(final Callable<R> block) {
     ...
     * 强行把操作放在子线程，主线程等待该子线程返回，
     */
    public static <R> R forceAsync(final Function<Throwable> exceptionHandler, ExecutorService executorService, final Callable<R> block) {
