...
/**
 * @author Jzs created 2017/8/5 0005
 * Glide缓存工具类
 */
...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(MyApplaciton.getInstance().getCacheDir() + "/" + AppConfig.GLIDE_CARCH_DIR)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(MyApplaciton.getInstance().getCacheDir() + "/" + AppConfig.GLIDE_CARCH_DIR, true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(MyApplaciton.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(MyApplaciton.getInstance()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(MyApplaciton.getInstance()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
